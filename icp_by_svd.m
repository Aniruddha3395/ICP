...
%     scatter3(start_scan_ptcloud(:,1),start_scan_ptcloud(:,2),start_scan_ptcloud(:,3),'.r')
    ...
    scan_ptcloud_transformed = [inv(Transform_mat)*[scan_ptcloud';ones(1,size(scan_ptcloud,1))]]';
    scan_ptcloud_transformed = scan_ptcloud_transformed(:,1:3);
    s0_transformed = [inv(Transform_mat)*[s0';ones(1,size(s0,1))]]';
    s0_transformed = s0_transformed(:,1:3);
    s0 = s0_transformed;
    Transform_mat_n = eye(4);
    Transform_mat_n(1:3,1:3) = Transform_mat(1:3,1:3);
    nx_transformed = [inv(Transform_mat_n)*[nx';ones(1,size(nx,1))]]';
    nx_transformed = nx_transformed(:,1:3);
    nx = nx_transformed;
    ny_transformed = [inv(Transform_mat_n)*[ny';ones(1,size(ny,1))]]';
    ny_transformed = ny_transformed(:,1:3);
    ny = ny_transformed;
    nz_transformed = [inv(Transform_mat_n)*[nz';ones(1,size(nz,1))]]';
